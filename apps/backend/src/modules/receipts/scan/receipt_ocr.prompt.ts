...
- If the next row is indented, attach it to the previous item
- Rows containing only quantity/unit-price details are not separate items
- Keep label as the product name only
- Put quantity into quantity
- Put per-unit price into unitPrice
- Put the final line price into totalPrice
- If a single price is clearly visible for an item, put it in totalPrice
- If a value is unclear, use null, not "-" or "unknown"
